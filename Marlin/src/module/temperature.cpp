...
void Temperature::mintemp_error(const heater_id_t heater_id OPTARG(ERR_INCLUDE_TEMP, const celsius_float_t deg)) {
  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    ...
    const bool is_idling = TERN0(HEATER_IDLE_HANDLER, heater_idle[ee].timed_out);
      ...
      const float pid_output = (!is_idling && temp_hotend[ee].is_above_target()) ? BANG_MAX : 0; // original: const float pid_output = (!is_idling && temp_hotend[ee].is_below_target()) ? BANG_MAX : 0;
          ...
          temp_bed.soft_pwm_amount = MAX_BED_POWER >> 1;
            ...
            if (WITHIN(temp_bed.celsius, BED_MINTEMP, BED_MAXTEMP)) {
                ...
                if (temp_bed.is_above_target(BED_HYSTERESIS))
                  temp_bed.soft_pwm_amount = 255; // Original = 0
                else if (temp_bed.is_below_target(BED_HYSTERESIS))
                  temp_bed.soft_pwm_amount = 0; // Original: temp_bed.soft_pwm_amount = MAX_BED_POWER >> 1;

              #else // !PIDTEMPBED && !BED_LIMIT_SWITCHING

                // Simple (noisy) "bang-bang" bed heating
                temp_bed.soft_pwm_amount = temp_bed.is_below_target() ? MAX_BED_POWER >> 1 : 0; // Original: temp_bed.soft_pwm_amount = temp_bed.is_below_target() ? MAX_BED_POWER >> 1 : 0; 
...
void Temperature::disable_all_heaters() {
  ...
  #if HAS_HEATED_BED
    setTargetBed(0); // Original setTargetBed(0);
    temp_bed.soft_pwm_amount = 0;
    WRITE_HEATER_BED(LOW);
  #endif
